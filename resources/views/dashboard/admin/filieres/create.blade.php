...
<div class="max-w-3xl rounded-3xl bg-white p-8 shadow-sm border border-slate-200">
    <div class="mb-6">
        <h1 class="text-2xl font-semibold text-slate-900">Ajouter une filière</h1>
        <p class="text-sm text-slate-500">Créez une nouvelle filière pour le catalogue de l’université.</p>
    </div>
    ...
    <form action="{{ route('admin.filieres.store') }}" method="POST" class="space-y-6">
        ...
        <div>
            <label class="block text-sm font-medium text-slate-700">Libellé de la filière *</label>
        ...
        <div>
            ...
            <textarea name="description" rows="5"
                class="mt-2 w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-100">{{ old('description') }}</textarea>
            @error('description')
            <p class="mt-2 text-sm text-rose-600">{{ $message }}</p>
            @enderror
        </div>
