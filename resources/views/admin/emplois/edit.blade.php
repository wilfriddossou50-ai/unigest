...
<div class="p-8">
    <div class="mb-8">
        ...
        <h1 class="mt-4 text-3xl font-bold text-slate-900">Modifier une séance</h1>
    </div>

    <div class="bg-white rounded-3xl border border-slate-200 shadow-sm p-8 max-w-3xl">
        <form action="{{ route('admin.emplois.update', $emploi) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <select name="matiere_id" required class="w-full rounded-lg border border-slate-200 px-4 py-2 focus:border-blue-500 focus:ring-blue-500">
                    ...
                    <option value="{{ $matiere->id }}" {{ old('matiere_id', $emploi->matiere_id) == $matiere->id ? 'selected' : '' }}>{{ $matiere->libelle ?? $matiere->code }}</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select name="professeur_id" required class="w-full rounded-lg border border-slate-200 px-4 py-2 focus:border-blue-500 focus:ring-blue-500">
                    ...
                    <option value="{{ $professeur->id }}" {{ old('professeur_id', $emploi->professeur_id) == $professeur->id ? 'selected' : '' }}>{{ $professeur->nom ?? '' }} {{ $professeur->prenom ?? '' }}</option>
                    @endforeach
                </select>
            ...
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                <div>
                    ...
                    <select name="semestre_id" required class="w-full rounded-lg border border-slate-200 px-4 py-2 focus:border-blue-500 focus:ring-blue-500">
                        ...
                        <option value="{{ $semestre->id }}" {{ old('semestre_id', $emploi->semestre_id) == $semestre->id ? 'selected' : '' }}>{{ $semestre->libelle }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="niveau_id" required class="w-full rounded-lg border border-slate-200 px-4 py-2 focus:border-blue-500 focus:ring-blue-500">
                        ...
                        <option value="{{ $niveau->id }}" {{ old('niveau_id', $emploi->niveau_id) == $niveau->id ? 'selected' : '' }}>{{ $niveau->libelle }}</option>
                        @endforeach
                    </select>
            ...
            <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
                <div>
                    ...
                    <select name="jour" required class="w-full rounded-lg border border-slate-200 px-4 py-2 focus:border-blue-500 focus:ring-blue-500">
                        ...
                        <option value="{{ $jour }}" {{ old('jour', $emploi->jour) == $jour ? 'selected' : '' }}>{{ $jour }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="time" name="heure_debut" value="{{ old('heure_debut', $emploi->heure_debut) }}" required class="w-full rounded-lg border border-slate-200 px-4 py-2 focus:border-blue-500 focus:ring-blue-500">
                ...
                <div>
                    ...
                    <input type="time" name="heure_fin" value="{{ old('heure_fin', $emploi->heure_fin) }}" required class="w-full rounded-lg border border-slate-200 px-4 py-2 focus:border-blue-500 focus:ring-blue-500">
            ...
            <div>
                ...
                <input type="text" name="salle" value="{{ old('salle', $emploi->salle) }}" placeholder="Ex: B101" required class="w-full rounded-lg border border-slate-200 px-4 py-2 focus:border-blue-500 focus:ring-blue-500">
            ...
            <div class="flex flex-col gap-4 pt-4 border-t border-slate-200 md:flex-row">
                <button type="submit" class="w-full rounded-lg bg-blue-600 px-4 py-3 text-white font-semibold transition hover:bg-blue-700">Mettre à jour</button>
