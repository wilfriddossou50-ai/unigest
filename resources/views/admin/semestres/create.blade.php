...
<div class="p-8 max-w-2xl">
    ...
    <h1 class="text-3xl font-bold text-slate-900 mb-8">Ajouter un Semestre</h1>
    <div class="bg-white rounded-xl shadow-sm p-8">
        <form action="{{ route('admin.semestres.store') }}" method="POST" class="space-y-6">
            @csrf
            <div>
                <label class="block text-sm font-semibold text-slate-900 mb-2">Niveau *</label>
                <select name="niveau_id" required
                    class="w-full px-4 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-blue-500 @error('niveau_id') border-red-500 @enderror">
            ...
            <div class="flex gap-4">
                <button type="submit" class="flex-1 bg-blue-600 text-white font-medium py-2 rounded-lg hover:bg-blue-700">Ajouter</button>
